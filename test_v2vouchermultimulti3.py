...
PARAMS = {
    ...
    Product.VOLCANIC_ROCK_VOUCHER_9500: { 
        ...
        "position_limit": 10, 
        "max_buy_premium": 250,  
        "stop_loss_underlying": 9650, 
        "profit_target_underlying": 10110,  
        "max_hold_time": 5,  
    },
    Product.VOLCANIC_ROCK_VOUCHER_10250: {  # Parameters for the option
    "strike": 10250,  # Strike price of the option
    "position_limit": 10,  # Limit on option positions
    "min_sell_premium": 500,  # Minimum premium to sell the option
    "stop_loss_underlying": 10100,  # Stop-loss on VOLCANIC_ROCK price
    "max_hold_time": 5,  # Maximum holding time for short position
    }
}
...
class Trader:
    def __init__(self, params=None):
        ...
        self.LIMIT = {
            ...
            Product.VOLCANIC_ROCK_VOUCHER_9500: 10,
            Product.VOLCANIC_ROCK_VOUCHER_10250: 10  # Added position limit for the option
        }
    ...
    def option_buy_strategy(
        ...
        """
        Strategy to buy undervalued options (VOLCANIC_ROCK_VOUCHER_9500).
        Buys the option if its price is low (low premium) and manages the position with a stop-loss
        and profit target based on the underlying price.
        """
        orders = []
        ...
        if current_position > 0:
            ...
            if underlying_mid_price <= stop_loss_underlying:
                # Close long position by selling the option
            ...
            # Check profit target based on underlying price
            profit_target_underlying = self.params[option_product]["profit_target_underlying"]
            if underlying_mid_price >= profit_target_underlying:
                # Close long position by selling the option
                quantity = abs(current_position)
                orders.append(Order(option_product, option_best_bid, -quantity))
                traderObject[position_key] = 0
        ...
        # Check for new long position if no current position
        if current_position == 0:
            ...
            # Buy the option if the premium is below the maximum threshold
            max_buy_premium = self.params[option_product]["max_buy_premium"]
            if premium <= max_buy_premium and position < position_limit:
                quantity = min(1, position_limit - position)  # Buy 1 unit at a time
                orders.append(Order(option_product, option_best_ask, quantity))
                traderObject[position_key] = quantity
                traderObject[entry_time_key] = timestamp
                traderObject[entry_price_key] = option_best_ask
    ...
    def run(self, state: TradingState):
        ...
        # Handle VOLCANIC_ROCK_VOUCHER_9500 with option buying strategy
        if Product.VOLCANIC_ROCK_VOUCHER_9500 in self.params and Product.VOLCANIC_ROCK_VOUCHER_9500 in state.order_depths:
            ...
            # Need the underlying order depth for stop-loss and profit target
            underlying_order_depth = state.order_depths.get(Product.VOLCANIC_ROCK, None)
            if underlying_order_depth:
                option_orders = self.option_buy_strategy(
                    Product.VOLCANIC_ROCK_VOUCHER_9500,
                ...
                result[Product.VOLCANIC_ROCK_VOUCHER_9500] = option_orders
        ...
        # Handle VOLCANIC_ROCK_VOUCHER_10250 with option selling strategy
        if Product.VOLCANIC_ROCK_VOUCHER_10250 in self.params and Product.VOLCANIC_ROCK_VOUCHER_10250 in state.order_depths:
            option_position = (
            ...
            if underlying_order_depth:
                ...
                result[Product.VOLCANIC_ROCK_VOUCHER_10250] = option_orders
