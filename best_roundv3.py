...
class Product:
    ...
    JAMS = "JAMS"
    DJEMBES = "DJEMBES"  # Nouvel actif ajouté

PARAMS = {
    Product.RAINFOREST_RESIN: {
        "fair_value": 10000,
        "take_width": 1.0,  # Augmenté de 0.75 à 1.0
        "clear_width": 1.0,  # Augmenté de 0.75 à 1.0
        "disregard_edge": 0.25,
        "join_edge": 3,
        "default_edge": 3,  # Augmenté de 2 à 3
        "soft_position_limit": 25,
    },
    Product.SQUID_INK: {
        "take_width": 4,  # Augmenté de 3 à 4
        "clear_width": 3,
        "prevent_adverse": True,
        "adverse_volume": 3,  # Réduit de 5 à 3
    ...
    Product.KELP: {
        "take_width": 3,  # Augmenté de 2 à 3
        "clear_width": 0,
        "prevent_adverse": True,
        "adverse_volume": 10,  # Réduit de 15 à 10
        "reversion_beta": -0.229,
        "disregard_edge": 2,
        "join_edge": 1,
        "default_edge": 4,
    ...
    Product.CROISSANTS: {
        ...
        "sinc_width": 500,
        "position_limit": 40,
        "buy_threshold": 0.2,  # Réduit de 0.3 à 0.2
        "sell_threshold": 0.02,  # Réduit de 0.05 à 0.02
        "max_spread": 10,
    },
    Product.JAMS: {
        "take_width": 1.5,
    ...
    Product.DJEMBES: {
        "volatility_window": 100,  # Augmenté de 75 à 100
        "position_limit": 40,
        "mean_reversion_strength": -0.5,
        "volatility_threshold_high": 0.00009,  # Augmenté de 0.000085 à 0.00009
        "volatility_threshold_low": 0.00005,  # Réduit de 0.000055 à 0.00005
        "max_spread": 10,  # Augmenté de 7 à 10
        "volatility_ma_window": 150,  # Augmenté de 100 à 150
    }
}
...
class Trader:
    def __init__(self, params=None):
        ...
        self.LIMIT = {
            ...
            Product.JAMS: 40,
            Product.DJEMBES: 40  # Limite de position pour DJEMBES
        }
    ...
    # Nouvelle méthode pour DJEMBES : Calcul de la volatilité roulante
    def compute_rolling_volatility(self, product: str, traderObject) -> float:
        price_history_key = f"{product}_price_history"
        if price_history_key not in traderObject or len(traderObject[price_history_key]) < 2:
            return 0.0
        
        prices = np.array(traderObject[price_history_key])
        window = self.params[product]["volatility_window"]
    ...
    # Stratégie de mean reversion pour DJEMBES basée sur la volatilité roulante
    def djembe_volatility_mean_reversion_strategy(
        self,
        product: str,
        order_depth: OrderDepth,
        traderObject,
        ...
        if price_history_key not in traderObject:
            traderObject[price_history_key] = []
        traderObject[price_history_key].append(mid_price)
        if len(traderObject[price_history_key]) > self.params[product]["volatility_window"]:
            traderObject[price_history_key] = traderObject[price_history_key][-self.params[product]["volatility_window"]:]

        # Calculer la volatilité roulante
        volatility = self.compute_rolling_volatility(product, traderObject)
        traderObject[f"{product}_volatility"] = volatility

        position_limit = self.params[product]["position_limit"]
        buy_quantity = position_limit - position
        sell_quantity = position_limit + position

        # Stratégie de mean reversion basée sur la volatilité
        volatility_high = self.params[product]["volatility_threshold_high"]
        volatility_low = self.params[product]["volatility_threshold_low"]

        if volatility > volatility_high and position > -position_limit:  # Vendre si volatilité élevée
            price = best_bid - 1
            if sell_quantity > 0:
                orders.append(Order(product, price, -sell_quantity))
        elif volatility < volatility_low and position < position_limit:  # Acheter si volatilité basse
            price = best_ask + 1
            if buy_quantity > 0:
                orders.append(Order(product, price, buy_quantity))
        else:  # Market making par défaut
            max_spread = self.params[product]["max_spread"]
            spread = max(1, min(max_spread, current_spread))
            ask = round(mid_price + spread / 2)
            bid = round(mid_price - spread / 2)
            if buy_quantity > 0:
                orders.append(Order(product, bid, buy_quantity))
    ...
    def run(self, state: TradingState):
        ...
        # Gestion de DJEMBES avec la stratégie basée sur la volatilité
        if Product.DJEMBES in self.params and Product.DJEMBES in state.order_depths:
            ...
            djembe_orders = self.djembe_volatility_mean_reversion_strategy(
                Product.DJEMBES,
                state.order_depths[Product.DJEMBES],
                traderObject,
                djembe_position,
            )
            result[Product.DJEMBES] = djembe_orders
