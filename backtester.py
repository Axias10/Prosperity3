...
class Backtester:
    ...
    def run(self):
        ...
        for timestamp, group in timestamp_group_md:
            ...
            local_bid = max(order_depths["MAGNIFICENT_MACARONS"].buy_orders.keys())
            local_ask = min(order_depths["MAGNIFICENT_MACARONS"].sell_orders.keys())
            implied_bid = local_bid - 2
            implied_ask = local_ask + 2
            observation = ConversionObservation("MAGNIFICENT_MACARONS", implied_bid, implied_ask)
            ...
            for product in products:
                ...
                if product == "MAGNIFICENT_MACARONS":
                    self._execute_conversion(conversions, order_depths_matching, self.current_position, self.cash, observation.conversionObservations["MAGNIFICENT_MACARONS"])
    ...
    def _construct_order_depths(self, group):
        """Construct order depths for all products from the current market data snapshot."""
        order_depths = {}  # Store order depths for each product
        for product in Product:
            if product != Product.GIFT_BASKET and product != Product.SPREAD:  # Exclude basket and spread
                # Get listings for this product
                listing = self.listings[product]
                # Get the current market data for this product
                data = group[group["product"] == listing.symbol]
                # Create OrderDepth object for this product
                order_depth = OrderDepth(listing)
                # Populate order depth with buy and sell orders
                for _, row in data.iterrows():
                    if row["side"] == "BUY":
                        order_depth.buy_orders[row["price"]] = row["quantity"]
                    else:
                        order_depth.sell_orders[row["price"]] = row["quantity"]
                # Store the order depth for this product
                order_depths[listing.symbol] = order_depth  # using listing.symbol as key
        return order_depths
